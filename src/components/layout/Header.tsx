      ...
      <div className="h-full px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative hidden md:block">
            ...
            <input
              type="text"
              placeholder="Search complaints, projects, roads..."
            ...
            />
            <kbd className="absolute right-3 top-1/2 -translate-y-1/2 px-2 py-0.5 bg-surface-700 rounded text-xs text-surface-400">
              ⌘K
            </kbd>
          </div>
        </div>
            ...
            <AnimatePresence>
              {showNotifications && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute right-0 top-full mt-2 w-80 bg-surface-800 border border-surface-700 rounded-xl shadow-xl overflow-hidden"
                >
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.map((notification) => (
                      <button
                        ...
                        className={cn(
                          "w-full p-4 text-left hover:bg-surface-700/50 transition-colors border-b border-surface-700/50 last:border-0",
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className={cn(
                            ...
                            notification.type === 'success' && "bg-accent-400",
                            notification.type === 'warning' && "bg-warning-400",
                            notification.type === 'info' && "bg-primary-400",
                            notification.type === 'error' && "bg-danger-400"
                          )} />
                          <div>
                            <p className="text-sm font-medium text-white">{notification.title}</p>
                            <p className="text-xs text-surface-400 mt-0.5">{notification.message}</p>
                          </div>
                        </div>
                      </button>
                    ))}
            ...
            <AnimatePresence>
              {showProfile && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute right-0 top-full mt-2 w-56 bg-surface-800 border border-surface-700 rounded-xl shadow-xl overflow-hidden"
                >
                  <div className="p-4 border-b border-surface-700">
